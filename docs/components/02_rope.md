...
这个方法有几个优点：简单、不需要学习参数、可以处理任意长度的序列。**但为什么同时使用正弦和余弦函数？** 这是为了在每个维度上提供两个相位偏移的信号。对于维度 $2i$ 和 $2i+1$，它们形成一个二维的旋转空间。利用三角函数的性质：
...
这意味着位置 $pos + k$ 的编码可以表示为位置 $pos$ 编码的线性组合。具体来说：

$$
\begin{bmatrix} PE_{(pos+k,2i)} \\ PE_{(pos+k,2i+1)} \end{bmatrix} = \begin{bmatrix} \cos(k\omega) & \sin(k\omega) \\ -\sin(k\omega) & \cos(k\omega) \end{bmatrix} \begin{bmatrix} PE_{(pos,2i)} \\ PE_{(pos,2i+1)} \end{bmatrix}
$$

其中 $\omega = 1/10000^{2i/d}$。这个线性变换关系允许模型学习到相对位置关系——通过学习固定的变换矩阵，模型可以从绝对位置编码中提取相对位置信息。
...
这是一个几何级数： $\theta_0 = 1, \theta_1 = \text{base}^{-2/d}, \theta_2 = \text{base}^{-4/d}, ...$ ，频率呈指数衰减。**为什么用几何级数？** 因为我们希望频率在对数尺度上均匀分布，这样可以覆盖从局部到全局的多个尺度。

**Base 值的选择也很重要**。常用的 base 是 10000（原始 Transformer）或 1000000（LLaMA）。Base 的大小直接影响最低频率分量的周期长度。对于 $d=128$ ：

- **base = 10000**：
  - 最高频： $\theta_0 = 1$ （周期 $2\pi \approx 6$ ）
  - 最低频： $\theta_{63} = 10000^{-126/128} \approx 0.0001$ （周期 $2\pi/0.0001 \approx 62832$ ）

- **base = 1000000**：
  - 最高频： $\theta_0 = 1$ （周期 $2\pi \approx 6$ ）
  - 最低频： $\theta_{63} = 1000000^{-126/128} \approx 0.00001$ （周期 $2\pi/0.00001 \approx 628318$ ）

**为什么 LLaMA 使用更大的 base？** 关键在于理解旋转角度的含义。对于相对位置差 $\Delta = n - m$，第 $i$ 对维度的旋转角度是 $\Delta \theta_i$。当 $\Delta$ 很大时（如在长序列中两个 token 相距很远），如果 $\theta_i$ 不够小，旋转角度 $\Delta \theta_i$ 会超过 $2\pi$ 多次，导致周期性重复，无法区分不同的相对距离。

具体来说：
- 对于 32K token 的序列，最大相对距离可达约 32000
- 如果 base = 10000，最低频率 $\theta_{63} \approx 0.0001$，那么 $32000 \times 0.0001 = 3.2$，对应的旋转角度约为 $3.2$ 弧度（未超过 $2\pi \approx 6.28$），勉强可以区分
- 如果 base = 1000000，最低频率 $\theta_{63} \approx 0.00001$，那么 $32000 \times 0.00001 = 0.32$，对应的旋转角度为 $0.32$ 弧度，远小于 $2\pi$，可以更好地编码长距离关系

因此，更大的 base 降低了最低频率，增大了周期长度，使得即使在很长的序列中，低频分量也不会因为周期性而混淆不同的相对位置。这就是为什么处理 32K 或更长上下文时需要更大 base 的原因。
