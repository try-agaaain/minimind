...
#### 6. [分词技术详解 (Tokenization)](./06_tokenization.md) ✅

**内容概览**：
- 分词的必要性和核心挑战（词表大小 vs 语义表达）
- 子词分词的演进：BPE、WordPiece、SentencePiece
- BPE 的工作原理和数学基础（字符对合并策略）
- WordPiece 的似然度优化方法（PMI 得分）
- SentencePiece 的语言无关设计和 Unigram 模型
- 实现细节：词表构建、编码解码、特殊 token 处理
- 实际应用考虑：词表大小选择、多语言处理、数字处理
- 现代 LLM 的分词选择（GPT、BERT、LLaMA 等）
...
**关键洞察**：
> 子词分词在词表大小和语义表达之间找到了优雅的平衡：用有限的词表覆盖无限的语言可能性，同时保留语义的可组合性。BPE 的频率驱动、WordPiece 的似然优化、SentencePiece 的语言无关，各有千秋。

---

#### 7. [LLM 损失函数设计 (Loss Function)](./07_loss_function.md) ✅

**内容概览**：
- 语言建模的本质：自回归预测和条件概率分解
- 交叉熵损失的信息论基础（熵、交叉熵、KL 散度）
- 数值稳定的 Softmax 和 Log-Softmax 实现
- 因果语言建模的特殊处理（输入-目标对齐、padding 忽略、注意力掩码）
- 困惑度（Perplexity）的含义和计算
- 高级技巧：Label Smoothing、辅助损失、长度归一化
- 实践技巧：梯度累积、梯度裁剪、损失尖峰监控
- 评估指标：Top-K 准确率、生成质量评估
...
**关键洞察**：
> 交叉熵损失不仅是一个损失函数，更是信息论和最大似然估计的优雅统一。理解其背后的数学原理——从熵到 KL 散度——能帮助我们更好地设计训练目标，调试问题，并探索新的优化方向（如 RLHF）。
...
**数据处理**：
6. Tokenization（已完成）→ 理解文本到数字的转换过程

**训练优化**：
7. Loss Function（已完成）→ 理解模型的学习目标和优化方向
...
- RoPE 的直觉部分
- SwiGLU 的基本概念
- Tokenization 的基本概念和算法思路
- Loss Function 的信息论基础

**中级**（需要线性代数和微积分基础）：
- RMSNorm 的完整推导
- RoPE 的数学证明
- GQA 的内存分析
- Tokenization 的算法实现细节
- Loss Function 的梯度计算和优化技巧

**高级**（需要深度学习和优化理论）：
- 反向传播的详细推导
- YaRN 的外推理论
- MoE 的负载均衡
- Tokenization 的 Unigram 语言模型方法
- Loss Function 的高级优化（RLHF、多目标优化）
...
| MoE | ✅ 完成 | 230 | 100% |
| Tokenization | ✅ 完成 | 395 | 100% |
| Loss Function | ✅ 完成 | 505 | 100% |
