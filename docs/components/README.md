...
#### 6. [模型评估](./06_evaluation.md) (430行) ✅

**内容概览**：
- 评估的必要性：训练损失的局限性
- 核心评估指标的数学原理和直觉理解
  - 困惑度（Perplexity）：等价分支因子的解释
  - Token 准确率：直接的预测质量度量
  - 交叉熵损失：优化目标与评估指标
  - 生成样本：定性评估的重要性
- 评估流程设计：数据加载、批处理、指标聚合
- 实现细节：模型加载、设备管理、数值稳定性
- 结果解读：如何理解各项指标的实际意义
- 最佳实践：独立评估集、随机种子、多次运行
...
**关键洞察**：
> 困惑度本质上是"等价分支因子"——困惑度为 N 意味着模型在预测下一个 token 时，平均需要在 N 个等可能的选项中选择。评估不仅要看数值指标，还要通过生成样本定性分析模型的实际能力。
