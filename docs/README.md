...
- **[RMSNorm：根均方归一化](./components/01_rmsnorm.md)** (160行) ✅
...
- **[RoPE：旋转位置编码](./components/02_rope.md)** (260行) ✅
...
- **[分组查询注意力 (GQA)](./components/03_attention.md)** (310行) ✅
  - MHA → MQA → GQA 的演进和设计权衡
  - KV Cache 机制的完整分析和内存计算
  - 内存和计算复杂度详细推导
  - Flash Attention 优化原理和实现
  - 注意力掩码的作用和实现细节
  
- **[SwiGLU 前馈网络](./components/04_swiglu_ffn.md)** (245行) ✅
  - FFN 激活函数的演进（ReLU → GELU → SwiGLU）
  - GLU 门控机制的数学基础和直觉理解
  - 中间层维度选择的原理（为什么是 8/3 倍）
  - 硬件对齐优化（为什么对齐到 64 的倍数）
  - 与注意力机制的类比和互补关系
  
- **[混合专家模型 (MoE)](./components/05_moe.md)** (230行) ✅
  - 稀疏激活和条件计算的理论
  - Top-K 路由机制的完整实现
  - 负载均衡问题和辅助损失详解
  - 共享专家的设计原理
...
> 💡 **特点**：每个文档都遵循教材级别的写作标准，从历史背景到数学推导到代码实现，step-by-step 详细展开，确保读者理解每一个细节。所有 5 个核心组件的详细文档已全部完成！

### 2. [LLM 分词技术完全指南：从原理到实践](./tokenization_evolution_guide.md) ⭐ 推荐

**系统性的分词技术教程**，从算法演进到实践应用的完整指南：

- **分词算法演进**
  - BPE：基于频率的合并策略
  - WordPiece：基于似然度增益的优化
  - Unigram Language Model (ULM)：基于概率的修剪方法
  - SentencePiece：语言无关的统一解决方案
  
- **实践指南**
  - transformers vs tokenizers 库的选择和使用
  - 训练自定义分词器的完整流程
  - 构建预训练数据集（CLM 和 MLM）
  - 统一数据格式 + 动态处理方案
...
> 💡 **特点**：从理论到实践的完整覆盖，包含大量代码示例、对比表格和决策树，适合想要深入理解分词技术并应用到实际项目的读者。

### 3. [模型架构概览](./minimind_architecture.md)

整体架构的快速概览（如需深入理解请查看上面的组件详解）

### 4. [训练指南](./training_guide.md)
