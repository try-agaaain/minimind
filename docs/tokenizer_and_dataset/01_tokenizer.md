...
在探索大语言模型的奥秘时，我们往往聚焦于 Transformer 架构、注意力机制等"高大上"的概念，却忽略了最基础也最关键的一环——**分词**。如果说 Transformer 是 LLM 的大脑，那么分词器就是它的感官系统，决定了模型如何"感知"和"理解"人类语言。
...
从另一个角度看，WordPiece 在最大化训练语料的似然度。**似然度**是概率论中的核心概念，衡量一个模型（此处是分词模型）对观测数据（语料）的解释程度。简单来说，它表示在给定模型参数下，观察到现有数据的"可能性"大小。在分词任务中，我们希望找到一个分词方案，使得训练语料在当前分词模型下的"出现"概率最大。假设我们用 Unigram 语言模型（token 独立出现），语料的对数似然是：

$$\log P(\text{corpus}) = \sum_{i=1}^{m} \log P(t_i)$$

其中 m 是语料中的 token 总数，P(t_i) 是第 i 个 token 的概率。对数似然越高，说明模型对数据的拟合越好。
...
**Unigram Language Model（单字语言模型）** 是一种概率模型，假设文本中的每个 token 都是独立生成的。在分词领域，Unigram LM 算法完全颠覆了 BPE 和 WordPiece 的构建思路。

BPE 和 WordPiece 都采用**自底向上**的策略：从字符开始，逐步合并。Unigram LM（Kudo 2018）完全反其道而行：**自顶向下**——从一个非常大的初始词汇表开始（通常通过从训练语料中提取所有唯一的字符和所有长度小于等于预设值的常见子字符串来构建），逐步删除不重要的子词，直到达到目标大小。
...
这是一个典型的 EM (Expectation-Maximization) 问题。让我们通过一个例子理解 EM 过程：

**假设**：初始词汇表包含 {"play", "ing", "p", "lay"}，对应概率为 {0.3, 0.2, 0.1, 0.1}（概率和为 1）

**E-step（期望步）**：给定当前概率，计算每个词的最优切分。对 "playing"：
- `["play", "ing"]` 概率 = 0.3 × 0.2 = 0.06
- `["p", "lay", "ing"]` 概率 = 0.1 × 0.1 × 0.2 = 0.002
选择概率更高的 `["play", "ing"]` 作为当前最优切分。

**M-step（最大化步）**：基于所有词的最优切分，重新估计概率。统计所有最优切分中每个子词的出现频率，归一化得到新概率。如果 "play" 在更多词中被选择，其概率会上升。
...
推理时，给定词 w，Unigram 需要找到概率最大的切分方式。这可以通过**Viterbi 算法**——一种动态规划算法——高效求解。Viterbi 算法通过构建一个图来寻找最优路径，其中节点代表可能的切分点，边代表可能的子词及其概率。
...
Viterbi 算法会构建从位置 0 到 7 的图，计算到每个位置的最大概率路径，最终选择概率更高的 `[_play, ing]`。
...
**SentencePiece** 不是一个新算法，而是一个**统一的分词框架**，它实现了 BPE 和 Unigram 算法，并解决了多语言处理的工程挑战。其核心创新是：**将文本视为原始字符流，包括空格**。

SentencePiece 用特殊符号 `▁`（U+2581，下划线方块）表示空格，将空格视为普通字符：
...
**基础控制标记**：
- `<unk>`：未知词，处理词汇表外的字符
- `<pad>`：填充标记，用于批处理时对齐序列长度
- `<s>` (BOS)：序列开始标记
- `</s>` (EOS)：序列结束标记，也是生成停止信号

**掩码语言模型专用**（BERT 系列）：
- `[CLS]`：分类标记，放在序列开头，其表示用于下游分类任务
- `[SEP]`：分隔符，用于分隔句子对（如问答中的问题和上下文）
- `[MASK]`：掩码位置，MLM 训练的核心

**对话模型专用**（ChatGPT、Llama-Chat）：
...
在保存的 Unigram 模型词表中，保存了每个词元（token）的对数概率（log probability）。表示它在语料中的出现概率，这些概率都是 0 到 1 之间的小数，取对数后就变成了负数。这样做的好处是方便概率的累乘和数值稳定性。简单来说，这些负数就是每个词元的“重要性”或“常见程度”，数值越接近 0，说明这个词元越常见。

当要将一段文本转为对应的token列表时，Unigram 分词器内部会比对所有可能的切分方案，用Viterbi 算法找到概率最大的那一种切分，返回给你对应的 token ID 序列。词表里的概率就是用来算出这个最大概率切分的。
...
回顾分词算法的演进，我们看到一条清晰的脉络：从 BPE 的简单频率统计，到 WordPiece 的关联性度量，再到 Unigram 的全局概率优化。这不是随机的技术轮换，而是对"什么是好分词"这个根本问题的认识深化。

**BPE** 告诉我们：高频模式值得成为独立 token，因为它们能有效压缩序列长度。

**WordPiece** 告诉我们：不仅仅是频率，统计关联性更能识别有语言学意义的单元。

**Unigram** 告诉我们：全局优化和概率建模能产生更平衡、更鲁棒的词汇表。

而 **SentencePiece** 则完成了工程上的统一，让这些算法真正适用于多语言场景。

理解这些演进背后的思想，不仅帮助我们选择合适的现成分词器，更重要的是，当面对特定领域（如医学文本、程序代码）时，我们能设计出更适合的分词策略。分词虽小，却是连接离散符号与连续表示的关键桥梁，值得我们深入理解和精心设计。
