"""
MiniMind LoRA (Low-Rank Adaptation) 训练脚本
用于对预训练模型进行参数高效的微调，只训练 LoRA 参数。
"""
import os
import argparse
import time
...
from my_lora import apply_lora, save_lora, load_lora, get_lora_params, count_lora_params, freeze_non_lora_params
...
class LoRATrainer:
    ...
    def _save_checkpoint(self, epoch, step):
        """保存 LoRA 检查点"""
        Logger(f"\n保存 LoRA 模型 (epoch {epoch+1}, step {step})...")
        ...
        Logger(f"✅ LoRA 权重已保存到: {lora_path}")
...
def main():
    ...
    # 保存配置
    parser.add_argument("--output_dir", type=str, default="./output/lora", help="模型保存目录")
    parser.add_argument("--lora_name", type=str, default="lora_sft", help="LoRA权重名称")
    
    # LoRA 配置
    parser.add_argument("--lora_rank", type=int, default=8, help="LoRA 的秩")
    parser.add_argument("--lora_alpha", type=float, default=16.0, help="LoRA 的缩放因子")
    ...
    # 数据配置
    parser.add_argument("--data_path", type=str, default="./dataset/lora_data.jsonl", help="LoRA训练数据路径")
    parser.add_argument("--tokenizer_path", type=str, default="./unigram_tokenizer.json", help="Tokenizer路径")
    
    # 预训练权重
    parser.add_argument('--from_weight', default='sft', type=str, help="基于哪个权重训练，默认 sft")
    ...
    if args.from_weight != 'none' and os.path.exists(args.from_weight):
        Logger(f"Loading base weights from {args.from_weight}")
    ...
    # ========== 6. 应用 LoRA ==========
    Logger(f"Applying LoRA with rank={args.lora_rank}, alpha={args.lora_alpha}")
    ...
    if args.use_wandb and is_main_process():
        try:
            ...
            wandb_run_name = f"MiniMind-LoRA-{args.lora_name}-Epoch-{args.epochs}-BS-{args.batch_size}-LR-{args.learning_rate}"
    ...
    if args.resume_from_checkpoint:
        ...
        if lora_path.exists():
            ...
            Logger(f"Loaded LoRA weights from {lora_path}")
